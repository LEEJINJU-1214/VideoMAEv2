...
def get_args():
    ...
    parser.add_argument(
        "--local_rank", default=int(os.environ.get("LOCAL_RANK", 0)), type=int
    )
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        for k in ["head.weight", "head.bias"]:
            if (
                ...
                if checkpoint_model[k].shape[
                    0
                ] == 710 and args.data_set.startswith("Kinetics"):
        ...
        if args.num_frames != 16:
            ...
            C = pos_tokens.shape[2]
            pos_tokens = pos_tokens.reshape(-1, T, P, P, C)
            pos_tokens = pos_tokens.permute(0, 2, 3, 4, 1).reshape(
                -1, C, T
            )  # BHW,C,T
            pos_tokens = torch.nn.functional.interpolate(
                pos_tokens, size=new_T, mode="linear"
            )
            pos_tokens = pos_tokens.reshape(1, P, P, C, new_T).permute(
                0, 4, 1, 2, 3
            )
