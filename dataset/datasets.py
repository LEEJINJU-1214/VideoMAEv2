    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            if len(buffer) == 0:
                while len(buffer) == 0:
                    warnings.warn(
                        "video {} not correctly loaded during validation".format(
                            sample
                        ),
                        stacklevel=2,
                    )
            ...
            if self.sparse_sample:
                spatial_step = (
                    1.0
                    * (
                        max(buffer.shape[1], buffer.shape[2])
            ...
            if seg_len <= converted_len:
                ...
                index = np.concatenate(
                    (
                        index,
                        np.ones(
                            self.clip_len - seg_len // self.frame_sample_rate
                        )
                        * seg_len,
                    )
                )
        ...
        if self.mode == "test":
            ...
            for idx in all_index:
                frame_fname = os.path.join(
                    fname, self.filename_tmpl.format(idx)
                )
        ...
        if average_duration > 0:
            if self.mode == "validation":
                ...
                all_index = list(
                    ...
                    + np.random.randint(
                        average_duration, size=self.num_segment
                    )
                )
